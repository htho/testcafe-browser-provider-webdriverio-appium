...
var _default = exports.default = {
  ...
  async openBrowser(id, url, browserName) {
    this._browser[id] = await (0, _webdriverio.remote)(this._config[browserName]);
   ...
   */
  async init() {
    const file = "file://" + _path.default.join(process.cwd(), "tcwdioappium.config.mjs");
  ...
  async resizeWindow(id, width, height, currentWidth, currentHeight) {
    const strategy = this._guessResizeStrategy(id, this._browser[id].requestedCapabilities["tcwdioappium:resizeStrategy"]);
    if (strategy === "rotateDevice") {
      // this seems to be the "official" command, but is not implemented by xcuitest-driver and safari-driver
      await this._browser[id].rotateDevice(0, 0, width > height ? 90 : 0);
    } else if (strategy === "setOrientation") {
      // setOrientation is deprecated, but works for iOS (17) anyway (xcuitest-driver).
      await this._browser[id].setOrientation(width > height ? "LANDSCAPE" : "PORTRAIT");
    } else {
      await this._setWindowSize(id, width, height, currentWidth, currentHeight);
    }
  },
  /**
   * @param {string} id A unique identifier generated by TestCafe that you can use to identify a browser window.
   * @param {import("../types").TcWdioAppiumOptions["tcwdioappium:resizeStrategy"]} strategy
   * @returns {import("../types").ResizeStrategies}
   */
  _guessResizeStrategy(id, strategy) {
    if (strategy !== undefined && strategy !== "auto") return strategy;

    /** @type {{platformName?: string}} */
    const capabilities = /** @type {any} */this._browser[id].capabilities;
    /** @type {string | undefined} */
    const platformName = capabilities.platformName;
    if (platformName === undefined) return "setWindowSize";
    if (platformName.toLowerCase() === "ios") return "setOrientation";
    if (platformName.toLowerCase() === "android") return "setOrientation";
  ...
  async _setWindowSize(id, width, height, currentWidth, currentHeight) {
    ...
    const currentWindowSize = (await this._browser[id].getWindowSize()) ?? {
